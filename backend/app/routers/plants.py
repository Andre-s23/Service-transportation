...
def list_plants(search: str = Query(""), db: Session = Depends(get_db), _: Employee = Depends(require_role(RoleEnum.admin, RoleEnum.manager, RoleEnum.driver))):
    q = db.query(Plant)
    if search: q = q.filter(Plant.name.ilike(f"%{search}%") | Plant.region.ilike(f"%{search}%"))
    return q.all()

@router.get("/{pid}", response_model=PlantResponse)
def get_plant(pid: int, db: Session = Depends(get_db), _: Employee = Depends(require_role(RoleEnum.admin, RoleEnum.manager))):
